...
public:
    ...
    void laserCloudInfoHandler(const lvi_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval) {
            ...
            auto kmStart = std::chrono::high_resolution_clock::now();
            extractSurroundingKeyFrames();
            auto kmEnd = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> kmDuration = kmEnd - kmStart;
            // std::cout << "提取关键帧点云耗时：" << kmDuration.count() << "ms" <<endl;
            ROS_INFO("keyframe clouds extraction cost : %.3f ms", kmDuration.count());

            scan2MapOptimization();
            auto smEnd = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> smDuration = smEnd - smStart;
            // std::cout << "scan to map 总耗时：" << smDuration.count() << "ms" << std::endl;
    ...
    void scan2MapOptimization()
    {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            // std::cout << "kdtree 构建耗时：" << kdDuration.count() << "ms" <<endl;
            ...
            auto smEnd = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> smDuration = smEnd - smStart;
            // std::cout << "优化耗时：" << smDuration.count() << "ms" <<endl;
