...
public:
    ...
    pcl::PointCloud<PointType>::Ptr laserCloudCornerFromMap;        // 子图点云
    ...
    void cornerOptimization()
    {
        ...
        for (int i = 0; i < laserCloudCornerLastDSNum; i++)
        {
            ...
            // std::vector<int> pointSearchInd;
            std::vector<float> pointSearchSqDis;
            vector<PointType, Eigen::aligned_allocator<PointType>> nearests;

            pointOri = laserCloudCornerLastDS->points[i];
            pointAssociateToMap(&pointOri, &pointSel);
            // kdtreeCornerFromMap->nearestKSearch(pointSel, 5, pointSearchInd, pointSearchSqDis);
            ikdtreeCornerFromMap->Nearest_Search(pointSel, 5, nearests, pointSearchSqDis, 1.0);
            ...
            if (pointSearchSqDis.size() == 5 && pointSearchSqDis[4] < 1.0) {
                ...
                for (int j = 0; j < 5; j++) {
                    ...
                    float ax = nearests[j].x - cx;
                    float ay = nearests[j].y - cy;
                    float az = nearests[j].z - cz;
    ...
    void surfOptimization()
    {
        ...
        for (int i = 0; i < laserCloudSurfLastDSNum; i++)
        {
            ...
            ikdtreeSurfFromMap->Nearest_Search(pointSel, 5, nearests, pointSearchSqDis, 1.0);
            ...
            if (pointSearchSqDis.size() == 5 && pointSearchSqDis[4] < 1.0) {
                for (int j = 0; j < 5; j++) {
                    ...
                    matA0(j, 0) = nearests[j].x;
                    matA0(j, 1) = nearests[j].y;
                    matA0(j, 2) = nearests[j].z;
                }
                ...
                for (int j = 0; j < 5; j++) {
                    ...
                    if (fabs(pa * nearests[j].x + pb * nearests[j].y + pc * nearests[j].z + pd) > 0.2) 
                    {
                        planeValid = false;
                        break;
                    }
    ...
    void scan2MapOptimization()
    {
        ...
        if (enoughSurf)
        {
            ...
            std::chrono::duration<double, std::milli> kdDuration = kdEnd - kdStart;
            ROS_INFO("kdtree construction cost : %.3f ms", kdDuration.count());*/
            ...
            auto smEnd = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> smDuration = smEnd - smStart;
            // ROS_INFO("LM optimization cost : %.3f ms", smDuration.count());
